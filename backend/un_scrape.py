...
async def linkpull(links, only_links):
    """
    ...
    for search_item in search_items:
        ...
        # Loop through each <a> element and get the href attribute
            ...
            count += 1
    print(count)
    return links, only_links

async def c_call(collection_name):
    return client.get_collection(name=collection_name)

async def c_del(collection_name):
    client.delete_collection(name=collection_name)
    

async def c_reset(collection_name):
    client.reset()


async def c_create(collection_name):

    return client.create_collection(name=collection_name)

async def embedder(links, collection_name, collection_action):
    collection = await collection_action(collection_name)
    ...
    for link in links:
        ...
        if response.status_code == 200:
            ...
            doc_content = await read_pdf(response.content)
...
async def scraper(keyword, name):
    client = chromadb.PersistentClient(path="./chroma", settings=Settings(allow_reset=True))
    if keyword not in client.list_collections():
        global driver
